...
export type DateContainerRecord = {
	id: string
	a: Date | null
	b: Date | null
}
...
export const createAuthor = (options: Partial<AuthorRecord> = {}): AuthorRecord => {
...
export const createAuthors = (options: {
	...
	return range(0, options.collectionSize <= 10 ** 12 ? options.collectionSize : 10 ** 12).map((i) => {
		const id = options.useSequentialIds ? `00000000-0000-0000-0000-${String(i).padStart(12, '0')}` : randomUUID()
		return createAuthor({
			id,
...
export const createDateContainers = (n: number, dateStep = 1): DateContainerRecord[] => {
